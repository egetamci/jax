...
def cloud_tpu_init() -> None:
  ...
  **This must be called before the TPU runtime is loaded, which happens as soon
  ...
  def is_in_cloud_tpu(libtpu_module, num_tpu_chips):
    return (
        libtpu_module is not None and num_tpu_chips > 0
    ) or jax_force_tpu_init()
  ...
  if not is_in_cloud_tpu(libtpu_module, num_tpu_chips):
    return
